...
sat_list_array=(CRYOSAT-2 CFOSAT ENVISAT ERS-1 ERS-2 GEOSAT GFO HY-2 HY-2B JASON-1
  JASON-2 JASON-3 SARAL SENTINEL-3A SENTINEL-3B SENTINEL-6A TOPEX SWOT)

for i in "${sat_list_array[@]}"; do
  fd -t f -a -e nc _"$i"_ $SRS_ALTI_DIR >$SRS_ALTI_MANIFEST_DIR/"$i".manifest
