function mainPigment()
%% mainPigment
% this processes all the XLS pigment files found in 'data_pigment.path' from config.txt
...
diary (strcat(DataFileFolder,filesep,readConfig('logFile.pigment.name', 'config.txt','=')));

for ii=1:length(xlsFiles)
    processPigment(DataFileFolder,xlsFiles(ii).name)
end
diary off
