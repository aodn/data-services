function [DATA,METADATA]=Absorption_CSV_reader(FileToConvert)
...
ii=1;
tline = fgetl(fid);
if ~isempty(strfind(tline,strcat('Table name',delimiter,'CF standard_name',delimiter,'IMOS long_name',delimiter,'Units',delimiter,'Fill value',delimiter,'Comments')))
    tline = fgetl(fid);
...
VarNameLine=DATA.VarName_Row{1};
for ii=2:length(DATA.VarName_Row)
    VarNameLine=strcat(VarNameLine,',',DATA.VarName_Row{ii});
end
...
if sum(strcmpi(VariableNames_Column, 'ad'))==1
    IndexMainVariable= strcmpi(VariableNames_Column, 'ad');
elseif sum(strcmpi(VariableNames_Column, 'ag'))==1
    IndexMainVariable= strcmpi(VariableNames_Column, 'ag');
    
elseif sum(strcmpi(VariableNames_Column, 'aph'))==1
    IndexMainVariable= strcmpi(VariableNames_Column, 'aph');
elseif sum(strcmpi(VariableNames_Column, 'ap'))==1
    IndexMainVariable= strcmpi(VariableNames_Column, 'ap');
else
    warning('variable not added in program')
end
IndexWavelengthVariable= strcmpi(VariableNames_Column, 'Wavelength');


tline = fgetl(fid);
NumberMainVariable=length(strfind(tline,char(DATA.VarName_Column{IndexMainVariable})));
NumberWavelengthVariable=length(strfind(tline,char(DATA.VarName_Column{IndexWavelengthVariable})));
