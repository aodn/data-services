...
for iiTag = 1:nTag
    ...
    pttCode              = unique(cell2mat({CTD_DATA((positionUniqueTagNames==iiTag)).PTT}'));
    
    END_DATE             = cell2mat({CTD_DATA(positionUniqueTagNames==iiTag).END_DATE}');
    [END_DATE_sorted,IX] = sort(END_DATE);
    
    lon_Tag              = cell2mat({CTD_DATA(positionUniqueTagNames==iiTag).LON}');
    lat_Tag              = cell2mat({CTD_DATA(positionUniqueTagNames==iiTag).LAT}');
    
    
    n_temp_Tag           = cell2mat({CTD_DATA(positionUniqueTagNames==iiTag).N_TEMP}'); % sometime N_TEMP can be NaN or N_SAL can be NaN. But not necessary both at the same time. So we get the value which is not NaN for the dimension
    n_sal_Tag            = cell2mat({CTD_DATA(positionUniqueTagNames==iiTag).N_SAL}');
    ...
    years                = (unique(cell2mat({METADATA.YEAR}')));
    location             = strrep(strrep(unique({METADATA.LOCATION}'),' ','_'),',','-');
    ...
    if length(years) == 1
        missionFolderName = strcat(num2str(years),'_',refcode_disassembled{1,1}{1},'_',locationAll);
    else
        missionFolderName = strcat(num2str(min(years)),'_',num2str(max(years)),'_',refcode_disassembled{1,1}{1},'_',locationAll);
    end
    ...
    if isempty(METADATA(WMO_index).WMO)
        ...
        if ~isempty(METADATA(WMO_index).WMO)
            WMO_NUMBER=strcat('Q9900',num2str(METADATA(WMO_index).WMO));
        else
            WMO_NUMBER = [];
        end
    else
        ...
        if N_DEPTH~=0
            if ~isnan(N_DEPTH)
                ...
                if ~(isnan(N_SAL) | N_SAL==0)
                    ...
                    SAL_VALS_profile_pre = SAL_VALS{1};
                    SAL_DBAR_profile     = (SAL_DBAR{1});
                ...
                if ~(isnan(N_TEMP) | N_TEMP==0)
                    ...
                    TEMP_VALS_profile_pre = TEMP_VALS{1};
                    TEMP_DBAR_profile     = TEMP_DBAR{1};
            ...
            missionPath    = strcat(dataWIP_Path,filesep,'NETCDF',filesep,missionFolderName );
