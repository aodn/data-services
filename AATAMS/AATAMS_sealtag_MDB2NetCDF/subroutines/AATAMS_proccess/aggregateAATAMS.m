...
for iiCruise =1:nCruise
    ...
    for i =1:nbprocessedfolder
        if (listfolder(i).isdir)
            if ( exist(strcat(dirprocessed,listCruise(iiCruise).name,filesep,listfolder(i).name,'/profiles'), 'dir') )
                
                listfiles = dir(strcat(dirprocessed,listCruise(iiCruise).name,filesep,listfolder(i).name,'/profiles/','*.nc'));
                ...
                if nbfiles~=0
                    ...
                    mkpath(strcat(dirprocessed,listCruise(iiCruise).name,filesep,listfolder(i).name,'/profiles/temporary/'));
                    for j = 1:nbfiles
                        %     for j =1:2
                        nc = netcdf.open(strcat(dirprocessed,listCruise(iiCruise).name,filesep,listfolder(i).name,'/profiles/',listfiles(j).name),'NC_NOWRITE');
                    ...
                    fileoutput = char(strcat(dirprocessed,listCruise(iiCruise).name,filesep,listfolder(i).name,'/profiles/temporary/',...
                    ...
                    netcdf.putAtt(nc,netcdf.getConstant('GLOBAL'),'author','Besnard, Laurent');
                    netcdf.putAtt(nc,netcdf.getConstant('GLOBAL'),'author_email','laurent.besnard@utas.edu.au');
                    ...
                    netcdf.close(nc);
                    %
                    filedirectionnetcdf = strcat(dirprocessed,listCruise(iiCruise).name,filesep,listfolder(i).name,filesep);
                    ...
                    fprintf('%s - WARNING: No Profiles for %s. Probably deleted by cleanNetCDF.m .Contact SMRU for more Information.\n',datestr(now),[listCruise(iiCruise).name filesep,listfolder(i).name])
                end
            end
        end
    end
