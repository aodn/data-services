...
if nargout == 0
    newplot;
    yo = y.*conj(y) / KMU /Fs * 2; % /Fs * 2 is modification to the original specgram
    yo(1) = yo(1)/2;  % modification to the original specgram
    if length(t)==1
        imagesc([0 1/f(2)],f,10*log10(yo+eps));axis xy; colormap(jet)
    else
        ...
        imagesc(t,f,10*log10(yo+eps));axis xy; colormap(jet)
    end
    xlabel('Time')
    ylabel('Frequency')
elseif nargout == 1,
