...
TEMP_PARAMETER_MAPPING = os.path.join(os.path.dirname(__file__), 'temp_parameters_mapping.csv')


def temp_data_parser(filepath):
    """
    parser of temp data file
    :param filepath: txt file path of AWAC temp data
...
def gen_nc_temp_deployment(deployment_path, metadata, site_info, output_path='/tmp'):
    ...
    tide_folder_path = os.path.join(deployment_path, "TEMPERATURE")
    data_temp_file_ls = ls_txt_files(tide_folder_path)

    missing_file_warn_str = 'No TEMPERATURE data files available in {path}'.format(path=deployment_path)
    if not data_temp_file_ls:
        logger.warning(missing_file_warn_str)
        return None
    elif "Temperature" not in data_temp_file_ls[0]:
        logger.warning(missing_file_warn_str)
        return None
    ...
    try:
        with Dataset(nc_file_path, 'w', format='NETCDF4') as nc_file_obj:
            nc_file_obj.createDimension("TIME", temp_data.datetime.shape[0])
            ...
            if df_varname_ls[0] == 'water_temp':
                ...
                dtype = temp_data[df_varname].values.dtype
