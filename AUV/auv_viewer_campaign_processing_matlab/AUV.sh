...
lockfile=${DIR}/${APP_NAME}.lock
{
  ...
  then
    ...
    if [ -e $touchfile ]; then 
        ...
        rsync --dry-run --size-only --itemize-changes --stats -uhvrD  --progress -a --include '*.nc' --include '*/r20*/hydro_netcdf'  --exclude='*.log'  --prune-empty-dirs  -e ${releasedCampaignPath}/ ${releasedCampaignOpendap}/;
        

        #rsync images from WIP to .thumbnails and remove from WIP
        rsync --dry-run --size-only --itemize-changes --stats   --progress --remove-source-files -a  --include="*/" --include="*.jpg" --exclude="*" --prune-empty-dirs  ${processedDataOutputPath}/ ${auvViewerThumbnailsPath}/;
        

        #rsync csv outputs used by talend from WIP to private
        rsync --dry-run --size-only --itemize-changes --stats   --progress  -a --exclude i2jpg/   --include '*.csv' --exclude '*.mat' --exclude '*.txt' --prune-empty-dirs  ${processedDataOutputPath}/ ${auvViewerThumbnailsPath}/;
