...
CREATE OR REPLACE VIEW report.anmn_bgc_data_summary_view AS
   SELECT
     ...
     (sum(n_ok)::float / sum(n_sample) * 100)::int AS percent_ok,
   ...
   FROM (SELECT *, 'Nutrients' AS product FROM anmn.nrs_hydall_view UNION
	 ...
	 SELECT *, 'Pigments' AS product FROM anmn.nrs_phypig_view  UNION
    	 SELECT site_code, sample_date, NULL, NULL, NULL, first_indexed, last_indexed, 'Field logsheets' as product FROM anmn.nrs_logsht_view  UNION
	 SELECT *, 'CTD' AS product FROM anmn.nrs_ctdpro_view  UNION
	 SELECT *, 'Zooplankton' AS product FROM anmn.nrs_zoo_view
        ) AS all_products
    	JOIN report.anmn_sites_view USING (site_code)
