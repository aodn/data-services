...
bathyRadical = bathyRadical.replace('10', '500')
...
# average 500mx500m (original is 10mx10m)
nMeanBlock = 500 / 10
nXBlock = nMeanBlock * nXBlock # has to be a multiple of nMeanBlock (nMeanBlock*10m = expected resolution) + quicker if also a multiple of original block size
nYBlock = nMeanBlock * nYBlock
...
outProj = Proj(proj='latlong',init='epsg:4326') # WGS84

# write output NetCDF file
nc = netCDF4.Dataset(bathyRadical + '.nc', 'w')

# create dimensions, variables and attributes:
iMeanBlock = nY/nMeanBlock
jMeanBlock = nX/nMeanBlock

nc.createDimension('Y', iMeanBlock)
nc.createDimension('X', jMeanBlock)
...
for i in xrange(0, nY, nYBlock):
    ...
    for j in xrange(0, nX, nXBlock):
        ...
        jjElem = range(0, cols, nMeanBlock)
        iiElem = range(0, rows, nMeanBlock)
        
        jMean = range(0, len(jjElem))
        iMean = range(0, len(iiElem))
        
        xMean    = np.empty(len(jjElem))
        yMean    = np.empty(len(iiElem))
        lonMean  = np.empty((len(iiElem), len(jjElem), ))
        latMean  = np.empty((len(iiElem), len(jjElem), ))
        dataMean = np.empty((len(iiElem), len(jjElem), ))

        for ii in iMean:
            for jj in jMean:
                xMean[jj] = np.nanmean(X[jjElem[jj]:jjElem[jj]+nMeanBlock-1])
                yMean[ii] = np.nanmean(Y[iiElem[ii]:iiElem[ii]+nMeanBlock-1])
                lonMean [ii, jj] = np.nanmean(lon [iiElem[ii]:iiElem[ii]+nMeanBlock-1, jjElem[jj]:jjElem[jj]+nMeanBlock-1])
                latMean [ii, jj] = np.nanmean(lat [iiElem[ii]:iiElem[ii]+nMeanBlock-1, jjElem[jj]:jjElem[jj]+nMeanBlock-1])
                dataMean[ii, jj] = np.nanmean(data[iiElem[ii]:iiElem[ii]+nMeanBlock-1, jjElem[jj]:jjElem[jj]+nMeanBlock-1])
        
        vertical_min = np.nanmin([np.nanmin(np.nanmin(dataMean)), vertical_min])
        vertical_max = np.nanmax([np.nanmax(np.nanmax(dataMean)), vertical_max])
        
        lat_min = np.nanmin([np.nanmin(np.nanmin(latMean)), lat_min])
        lat_max = np.nanmax([np.nanmax(np.nanmax(latMean)), lat_max])
        lon_min = np.nanmin([np.nanmin(np.nanmin(lonMean)), lon_min])
        lon_max = np.nanmax([np.nanmax(np.nanmax(lonMean)), lon_max])
    
        # write lon,lat,depth
        nc_x[j/nMeanBlock:(j+cols)/nMeanBlock] = xMean[0:cols/nMeanBlock]
        nc_y[i/nMeanBlock:(i+rows)/nMeanBlock] = yMean[0:rows/nMeanBlock]
        nc_lon[i/nMeanBlock:(i+rows)/nMeanBlock, j/nMeanBlock:(j+cols)/nMeanBlock] = lonMean[0:rows/nMeanBlock, 0:cols/nMeanBlock]
        nc_lat[i/nMeanBlock:(i+rows)/nMeanBlock, j/nMeanBlock:(j+cols)/nMeanBlock] = latMean[0:rows/nMeanBlock, 0:cols/nMeanBlock]
        
        # replace NaNs by _FillValue
        dataMean[np.isnan(dataMean)] = np.float32(99999)
        
        nc_height[i/nMeanBlock:(i+rows)/nMeanBlock, j/nMeanBlock:(j+cols)/nMeanBlock] = dataMean[0:rows/nMeanBlock, 0:cols/nMeanBlock]
...
nc.title = 'Victorian Coastal Digital Elevation Model (VCDEM 2017) 500m resolution.'
...
nc.abstract = ('A gap free Digital Elevation Model (DEM) for the Victorian '
               ...
               'Coastal Region.')
nc.lineage = ('This 500m DEM has been derived from the Continuous Seamless 10m DEM which is derived from the Updated '
