...
rsync -avr --remove-source-files ${RSYNC_SOURCE_RAW_PATH}/ ${RSYNC_DEST_RAW_PATH}/

# rsync between staging and opendap : move data to opendap
rsync -avr --remove-source-files --include '+ */' --include '*.nc' --exclude '*.png'  ${RSYNC_SOURCE_PATH}/ ${RSYNC_DESTINATION_PATH}/
