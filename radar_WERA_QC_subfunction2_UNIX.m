function [dateforfileSQL] = radar_WERA_QC_subfunction2_UNIX(namefile,site_code,zz)
...
%
global dfradialdata
%reminder: dfradialdata ='/home/matlab_3/datafabric_root/opendap/ACORN/radial_quality_controlled/';
%see matlab script 'radar_WERA_QC_main_UNIX.m' for any changes
global inputdir
%reminder: inputdir = '/var/lib/matlab_3/ACORN/WERA/radial_QC/';
%see matlab script 'radar_WERA_QC_main_UNIX.m' for any changes
global outputdir
%reminder: outputdir = '/var/lib/matlab_3/ACORN/WERA/radial_QC/output/';
%see matlab script 'radar_WERA_QC_main_UNIX.m' for any changes
global ncwmsdir
%reminder: ncwmsdir = '/var/lib/netcdf_data/matlab_3/ncwms.emii.org.au_ncwms_data/WERA_QC/';
%see matlab script 'radar_WERA_QC_main_UNIX.m' for any changes
%
...
station1 = NaN(maxPOS,10,7);
station2 = NaN(maxPOS,10,7);
%
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%Quality control information of the Current Speed
        station1(POS(j),10,t) = speedQC(j);
%
    end
    t=t+1;
end
clear POS lat lon speed dir
clear error bragg speedQC
%%%%%%%%%%%%%%%%%%DATA CHECK%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
...
for i=1:6
    M = find(station1(:,10,i)<1 & station1(:,10,i)>2);
    station1(M,:,i) = NaN;
end
clear M
...
%QC CRITERIA
for i =1:6
    M = find(station2(:,10,i)<1 & station2(:,10,i)>2);
    station2(M,:,i) = NaN;
end
clear M
%
...
if (~isempty(final))
%
%NECESSITY TO IMPORT THE LATITUDE AND THE LONGITUDE VALUES OF THE OUTPUT
%GRID 
switch site_code
    case {'SAG','CWI','CSP'}
...
netcdfoutput = strcat(pathoutput,'IMOS_ACORN_V_',dateforfileSQL,'Z_',site_code,'_FV01_1-hour-avg.nc');
%
...
switch site_code
    case {'SAG','CWI','CSP'}
      netcdf.putatt(nc,netcdf.getConstant('GLOBAL'),'title',strcat('IMOS ACORN South Australia Gulf (SAG), one hour averaged current QC data, ',datestr(timenc(1)+datenum(timestart)+1/48,'yyyy-mm-ddTHH:MM:SSZ')));    
    ...
    case {'GBR','TAN','LEI','CBG'}
      netcdf.putatt(nc,netcdf.getConstant('GLOBAL'),'title',strcat('IMOS ACORN Capricorn Bunker Group (CBG), one hour averaged current QC data, ',datestr(timenc(1)+datenum(timestart)+1/48,'yyyy-mm-ddTHH:MM:SSZ')));    
    ...
    case {'PCY','FRE','GUI'}
      netcdf.putatt(nc,netcdf.getConstant('GLOBAL'),'title',strcat('IMOS ACORN Rottnest Shelf (ROT), one hour averaged current QC data, ',datestr(timenc(1)+datenum(timestart)+1/48,'yyyy-mm-ddTHH:MM:SSZ')));    
...
acorncomment = ['The ACORN facility is producing NetCDF files with radials data for each station every ten minutes. '...
    ' Each radial value have a corresponding quality control flag.'...
...
switch site_code
    case {'SAG','CWI','CSP'}
        pathoutput = strcat(outputdir,'datafabric/gridded_1havg_currentmap_QC/SAG/');
    case {'GBR','TAN','LEI','CBG'}
        pathoutput = strcat(outputdir,'datafabric/gridded_1havg_currentmap_QC/CBG/');
    case {'PCY','FRE','GUI','ROT'}
        pathoutput = strcat(outputdir,'datafabric/gridded_1havg_currentmap_QC/ROT/');
 end
%
...
%
netcdfoutput = strcat(pathoutput,yearDF,'/',monthDF,'/',dayDF,'/','IMOS_ACORN_V_',dateforfileSQL,'Z_',site_code,'_FV01_1-hour-avg.nc');
%
nc = netcdf.create(netcdfoutput,'NC_CLOBBER');
%
%Creation of the GLOBAL ATTRIBUTES
...
netcdf.putatt(nc,netcdf.getConstant('GLOBAL'),'naming_authority','IMOS');
netcdf.putatt(nc,netcdf.getConstant('GLOBAL'),'quality_control_set','1');
netcdf.putatt(nc,netcdf.getConstant('GLOBAL'),'file_version','Level 1 - Quality Controlled data');
acornQC = ['Data in this file has been through the IMOS quality control procedure (Reference Table C). '...
       ' Every data point in this file has an associated quality flag'];
netcdf.putatt(nc,netcdf.getConstant('GLOBAL'),'file_version_quality_control',acornQC);
%WHERE
netcdf.putatt(nc,netcdf.getConstant('GLOBAL'),'geospatial_lat_min',min(Y));
netcdf.putatt(nc,netcdf.getConstant('GLOBAL'),'geospatial_lat_max',max(Y));
...
netcdf.putatt(nc,netcdf.getConstant('GLOBAL'),'comment',acorncomment);
netcdfabstract = [' The ACORN facility is producing NetCDF files with radials data for each station every ten minutes. '...
...
%
   ...
   warningmessage = 'no gridded data for this particular date and time'
end
