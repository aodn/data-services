...
%Turquoise Coast
try
    radar_CODAR_non_QC_to_ncWMS_main_UNIX_v1('TURQ')
catch
...
try
    radar_CODAR_non_QC_to_ncWMS_main_UNIX_v1('BONC')
catch
    fid_w = fopen(logfile,'a');
    fprintf(fid_w,'%s %s \r\n',datestr(clock),' PROBLEM to PROCESS DATA FOR THE RADAR SITE BONC ');
    fclose(fid_w);
end
