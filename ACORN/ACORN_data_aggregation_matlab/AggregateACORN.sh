...
touchfile=/tmp/running_ACORN-agg
...
if [ -e $touchfile ]; then 
	...
	${pythonPath} ${scriptpath}"/subroutines/AggregateACORN.py" 2>&1 | tee  /tmp/log_ACORN-agg.log

	#send email of log
	mail -s "AggregateACORN job"  ${email1} -v < /tmp/log_ACORN-agg.log;
	mail -s "AggregateACORN job"  ${email2} -v < /tmp/log_ACORN-agg.log;
	rm $touchfile
fi