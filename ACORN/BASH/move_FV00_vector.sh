...
find $STAGING/ACORN/acorn-migration-hierarchy/vector/ -type f -amin +5 -name "*FV00_sea-state.nc" -printf %P\\n | sort > /tmp/move_FV00_vector.$$.list
...
cat /tmp/move_FV00_vector.$$.list | xargs -I {} isCorruptedNC.sh $STAGING/ACORN/acorn-migration-hierarchy/vector/{} /tmp/isCorruptedNC.$$.list
