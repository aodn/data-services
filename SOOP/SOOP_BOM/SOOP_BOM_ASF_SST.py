...
if __name__ == "__main__":
    ...
    # local storage
    temporaryDataFolderUnsorted = config.get('temporaryDataFolderUnsorted.path')
    temporaryDataFolderSorted   = config.get('temporaryDataFolderSorted.path')
    ...
    try:
        cmd =  ('lftp -u '+ \
                 bom_ftp_username +',' +\
                 bom_ftp_password +\
                 ' -e \'mirror ' + bom_ftp_subdir +' '+  temporaryDataFolderUnsorted  + '\' ' +\
                 bom_ftp_address)
        msg = os.system(cmd)
    except Exception, e:
        print str(e)
    ...
    try:
      ...
      print "ERROR: uncaught error occured with FileSort " +temporaryDataFolderUnsorted + os.path.sep +  "ships"+ os.path.sep + str(e)
      pass
    filesort.close()
