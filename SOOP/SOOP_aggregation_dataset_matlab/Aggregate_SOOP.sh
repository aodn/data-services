...
touchfile=/tmp/running_SOOP-agg
...
if [ -e $touchfile ]; then 
	...
	${pythonPath} ${scriptpath}"/subroutines/AggregateSOOP.py" 2>&1 | tee  /tmp/log_SOOP-agg.log

	#send email of log
	mail -s "AggregateSOOP job"  ${email1} -v < /tmp/log_SOOP-agg.log
	mail -s "AggregateSOOP job"  ${email2} -v < /tmp/log_SOOP-agg.log;
	rm $touchfile
fi