function performAggregationFromList(fileList,soopSubFacility)
...
% used files (NCML and NetCDF) are deleted from the working directory.
%
% Syntax:  performAggregationFromList
%
% Inputs:   fileList        : list of files to aggregate together
%           soopSubFacility : soop sub facility to aggregate
%
% Outputs:
%
%
...
%
% See also:
% Aggregation_Sub_SOOP,readConfig,aggregateFiles
%
...
%% change QC var into Bytes and not double !! if  quality_control_set: 1 (IMOS IODE flag)
idxQC= ~cellfun('isempty',(regexp(allVarnames,'_quality_control')));
qcVarName=allVarnames(idxQC);
nQCVAR=length(qcVarName);

if nQCVAR > 0
    quality_control_set = netcdf.getAtt(nc,netcdf.inqVarID(nc,qcVarName{1}),'quality_control_set');
    if  quality_control_set == 1 
...
titleNetCDF=readConfig(strcat('gAttTitle.',soopSubFacility), 'config.txt','=');
abstractNetCDF=readConfig(strcat('gAttAbstract.',soopSubFacility,'.',aggregationType),'config.txt','=');
...
write_NC_Command=sprintf('%s -classpath ".:%s/lib/java/class/toolsUI-4.2.jar"  AggregateNcML %s %s',readConfig('java.path', 'config.txt','='),SCRIPT_FOLDER,ncmlFile,fullfile(TEMPORARY_FOLDER,outputNETCDF));
...
if status ==0 % 0 is success , 1 is not , pretty weird matlab standard
    ...
    platform_code=gattvalOld{strcmpi(gattnameOld,'platform_code')};
    % platform_code=char(soopSubFacility);
    switch aggregationType
        case 'month'
            IMOS_NAME_freePartCode=strcat(platform_code(1:3),'_monthly-agg');
        case 'year'
            IMOS_NAME_freePartCode=strcat(platform_code(1:3),'-yearly-agg');
    end
    
    [numOffset,~,FirstDate,LastDate]= getTimeOffsetNC(ncAggregated);
    ...
    LAT=getVarNC('latitude',ncAggregated);
    LON=getVarNC('longitude',ncAggregated);
    ...
    gattname{20}='cdm_data_type';
    gattval{20}=readConfig('gAttVal.cdm_data_type', 'config.txt','=');
    ...
    [allGattName]=getGlobAttNC(ncAggregated);
    if ismember('voyage_number',allGattName)
        netcdf.delAtt(ncAggregated,netcdf.getConstant('GLOBAL'),'voyage_number')
    end
    ...
    aggregatedShipFolder=strcat(TEMPORARY_FOLDER,filesep,'aggregated_datasets',filesep,pathFile);
    mkpath(aggregatedShipFolder)
    
    movefile(fullfile(TEMPORARY_FOLDER,outputNETCDF),fullfile(aggregatedShipFolder,filesep,modifiedFileName))
    delete(ncmlFile)
    ...
    save(fullfile(TEMPORARY_FOLDER,'alreadyAggregated.mat'), '-regexp','fileAlreadyUsed')
    
    %% we delete the files from filelist
    for iiFile=1:nFilelist
                if exist(char(fileList(iiFile)),'file')==2
        delete(char(fileList(iiFile))); %ncml +nc
                end
        
                if exist(char(strcat(filepath(iiFile),filesep,name(iiFile),'.nc')),'file')==2
