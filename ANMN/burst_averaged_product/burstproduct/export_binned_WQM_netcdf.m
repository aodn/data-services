...
abstract_string_suffix=[' Water quality meters (WQM''s) at one or more depths collect bursting data. Data '...
            'from the bursts have been cleaned and averaged to create data products. ' ...
...
lineage_string=['The data array in this file has been created by binning (averaging) raw burst data. '... 
                 'Each array value is the arithmetic mean of a burst. Length and frequency ' ...
                 'of the underlying bursts are declared in the global attributes instrument_burst_duration' ...
                 ' and instrument_burst_interval respectively. Out-of-water data is excised before binning. '...
                 ...
                 'this includes data values outside of a valid range,' ...
                    ...
                    'whose length exceeds a maximum. For all parameters except those collected by optical '...
                    'sensor (TURB, CPHL, CHLR, CHLU, CHLF, FLU2), points flagged by IMOS QC as 3 '...
                    'are also removed. These flags are produced by the IMOS rate of change test: this is '...
                    ' a test to detect spikes. This sort of test is not used to exclude any optical sensor '...
                    'data, because understanding of behaviour of these parameters at this time scale is still '...
                    ' in its early stages. Burst means are calculated' ...
                    ...
                    ' value of FillValue).']; 
lineage_MaIs_suffix=[' For Maria Island PSAL, data values with a rate of change test result '...
                    ' more extreme than the 99.9th percentile of rate of change test values (after taking'...
                    ' into account time of year and depth) are also removed. The time value is assigned the point half-way' ...
                    ' between the start and end points of the raw burst.'];
...
if isfield(global_attributes,'site_code')
    ...
    disp('Missing site name in attributes.')
    deployment_location='site name?';
end
title_string=['Burst-averaged biogeochemical measurements at',' ',deployment_location];
...
for i=1:length(global_attribute_names)
    if ~strcmp(global_attribute_names{i},'title')              % it appears that putAtt overwrites previous values if you do it >1 times
        ...
        if strcmp(global_attribute_names{i},'comment')             % insert after the comment:
            if strfind(filename,'NRSMAI')
                lineage_string=strcat(lineage_string,lineage_MaIs_suffix);
            end
