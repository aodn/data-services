function [alreadyDownloaded,channelInfo,filebroken]=downloadChannelNRS(channelIDToProcess,alreadyDownloaded,channelInfo,level)
%% downloadChannelNRS
...
if ~isempty(START) && ~isempty(STOP)  % START and STOP will be empty if one file has already been downloaded for the current day
    ...
    for j=1:size(START,2)             % j is the number of files to download for each channel
        ...
        if AIMS_server_online==0
            ...
            LogChannelIDdown_DATES=fullfile(NRS_DownloadFolder,strcat('log_ToDo/ChannelID_downDates_',DATE_PROGRAM_LAUNCHED,'.txt'));
        ...
        if ~isempty(filenameUnrenamed) && ~isempty(filepath)
            ...
            if ~isNetCDFempty(strcat(filepath,filename))
                Move_File_NRS(channelIDToProcess,channelInfo.siteName{channelIDToProcess},channelInfo.parameterType{channelIDToProcess},alreadyDownloaded.folderLongnameDepth{channelIDToProcess},yearFile,filename,filepath,level,DATE_PROGRAM_LAUNCHED);
                ...
                Move_brokenFile_NRS(channelIDToProcess,channelInfo.siteName{channelIDToProcess},channelInfo.parameterType{channelIDToProcess},alreadyDownloaded.folderLongnameDepth{channelIDToProcess},yearFile,filename,filepath,level);
        ...
        if    Last2Delete==1 && level==0 && ~isempty(filename) && filebroken==0
            ...
            DeleteFile_NRS(channelIDToProcess,channelInfo.siteName{channelIDToProcess},channelInfo.parameterType{channelIDToProcess},alreadyDownloaded.folderLongnameDepth{channelIDToProcess},yearFile2Delete,File2Delete,level,DATE_PROGRAM_LAUNCHED);
