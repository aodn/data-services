...
if skip==0
    ...
    if dimlenTIME >0
        ...
        CreationDate=datestr(now+datenum(0,0,0,0,0,60)-datenum(0,0,0,localTimeZoneComputer,0,0),'yyyymmddTHHMMSS');%add 60 seconds in case the programm is to fast and add the same filename
        ...
        ExprToRead=strcat('Z\+');%in case there is a Z we remove it
        filenameNew=regexprep(filenameNew, ExprToRead, '+','ignorecase');
        
        ExprToRead=strcat('\+[\d.]+_');
        filenameNew=regexprep(filenameNew, ExprToRead, '_','ignorecase'); % if there is a + , it is incoherent. Cannot have a Z (UTC time) and a local time zone +10, so we remove it in case AIMS does not do it
        ...
        TIME_VARNAME='TIME';
        DEPTH_VARNAME='DEPTH'; %variable to remove because this was the same value all the time, and when mathias was here we didn't want this for ramadda
        try
