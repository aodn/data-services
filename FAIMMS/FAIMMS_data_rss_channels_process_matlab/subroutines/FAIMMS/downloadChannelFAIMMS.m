function [alreadyDownloaded,channelInfo,filebroken]=downloadChannelFAIMMS(channelIDToProcess,alreadyDownloaded,channelInfo,levelQC)
    ...
    for j=1:size(startDate,2)             % j is the number of files to download for each channel
        ...
        if ~isempty(filenameUnrenamed) && ~isempty(filepath)
            ...
            if ~isNetCDFempty(strcat(filepath,filename))
                Move_File(channelIDToProcess,channelInfo.siteName{channelIDToProcess},channelInfo.siteType{channelIDToProcess},channelInfo.parameterType{channelIDToProcess},alreadyDownloaded.folderLongnameDepth{channelIDToProcess},yearFile,filename,filepath,levelQC,DATE_PROGRAM_LAUNCHED);
                ...
                Move_brokenFile(channelIDToProcess,channelInfo.siteName{channelIDToProcess},channelInfo.siteType{channelIDToProcess},channelInfo.parameterType{channelIDToProcess},alreadyDownloaded.folderLongnameDepth{channelIDToProcess},yearFile,filename,filepath,levelQC);
        ...
        if    lastDateToDeleteBoolean==1 && levelQC==0 && ~isempty(filename) && filebroken==0
            ...
            DeleteFile(channelIDToProcess,channelInfo.siteName{channelIDToProcess},channelInfo.siteType{channelIDToProcess},channelInfo.parameterType{channelIDToProcess},alreadyDownloaded.folderLongnameDepth{channelIDToProcess},yearFile2Delete,File2Delete,levelQC,DATE_PROGRAM_LAUNCHED);
